...
3. For image, video, and audio, select the UMA protocol and model target that match the provider API.
...
If the protocol is absent, do not select an approximately similar protocol. Add a UMA V2 protocol and the corresponding media target from the provider API documentation first. See [Model configuration and provider protocols](./model-providers.md) for every field and troubleshooting.
