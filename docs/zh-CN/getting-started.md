...
1. 打开右上角设置面板。
2. 在 LLM 页面添加至少一个文本模型，并设置所属档位和档位默认。
3. 按需在媒体页面添加图片、视频或音频服务。
4. 视频 Provider 显式选择 UMA 协议和模型目标；图片或音频选择对应 OpenReel Catalog 协议。
5. 保存后新建项目，用一句简单请求验证 Agent，再用最小媒体节点验证外部生成服务。

配置真相源是本地 `config/runtime.jsonc`。优先通过设置面板修改，不要把真实 API Key 提交到 Git。

设置页没有独立的连接测试按钮；保存表示本地配置校验通过，真实可用性由节点最小运行验证。字段填写、Base URL 规则、视频 UMA 边界和图片/音频 Catalog 说明见[模型配置与协议接入](./model-providers.md)。
...
LLM 与媒体服务是独立配置。检查对应媒体 Provider 是否启用、模型名是否正确、协议 ID 是否存在，再用协议支持的最小参数运行一个节点并阅读节点错误。
