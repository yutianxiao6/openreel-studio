...
4. 为媒体 Provider 选择与服务商 HTTP 接口匹配的协议；如果下拉框没有对应协议，先补充协议 Catalog。
5. 保存后新建项目，用一句简单请求验证 Agent，再用最小媒体节点验证外部生成服务。

配置真相源是本地 `config/runtime.jsonc`。优先通过设置面板修改，不要把真实 API Key 提交到 Git。

设置页没有独立的连接测试按钮；保存表示本地配置校验通过，真实可用性由节点最小运行验证。字段填写、Base URL 规则以及图片/视频/音频协议写法见[模型配置与协议接入](./model-providers.md)。
