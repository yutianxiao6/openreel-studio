...
_PROJECT_DELETE_ACTIONS = {"delete", "remove", "rm", "del", "删除"}
...
async def slash_command_events(
    ...
    *,
    ...
    if command.name == "project":
        async for event in _project_events(project_id, command):
            yield event
        return
...
async def _project_events(
    ...
    if action in _PROJECT_LIST_ACTIONS:
        projects = await _list_project_records()
        text = _format_project_list(projects, current_project_id=project_id)
        await _emit_text(project_id, command, text)
        yield {
            "type": "slash_command",
            "command": "project",
            "action": "list",
            "ok": True,
            "result": {"projects": projects, "current_project_id": project_id},
        }
        yield {"type": "text_delta", "content": text}
        yield {"type": "done", "status": "completed"}
        return

    if action in _PROJECT_NEW_ACTIONS:
        title = " ".join(command.args[1:]).strip() or "未命名项目"
        project = await _create_project_record(title)
        _emit_control_plane_event(
            project_id,
            command,
            branch="project_created",
            status="completed",
            created_project_id=project["id"],
            title=project["title"],
        )
        text = f"已新建并切换到项目：{project['title']}。"
        await _save_message(
            project["id"],
            "user",
            command.raw,
            {"source": "slash_command", "command": command.name, "mirrored_from_project_id": project_id},
        )
        await _save_message(
            project["id"],
            "assistant",
            text,
            {"source": "slash_command", "command": "project", "ok": True, "action": "new"},
        )
        yield {"type": "project_switch", "project_id": project["id"], "title": project["title"], "action": "created"}
        yield {
            "type": "slash_command",
            "command": "project",
            "action": "new",
    ...
    if action in _PROJECT_SWITCH_ACTIONS:
        query = " ".join(command.args[1:]).strip()
        projects = await _list_project_records()
        target, error_text = _resolve_project_record(projects, project_id, query)
        if not target:
            text = error_text or "未找到项目。用 /project 查看可切换项目。"
            await _emit_text(project_id, command, text, ok=False)
            yield {"type": "slash_command", "command": "project", "action": "switch", "ok": False, "error": "project_not_found"}
            yield {"type": "text_delta", "content": text}
            yield {"type": "done", "status": "failed"}
            return
        _emit_control_plane_event(
            project_id,
            command,
            branch="project_switched",
            status="completed",
            target_project_id=target["id"],
            title=target["title"],
        )
        text = f"已切换到项目：{target['title']}。"
        ...
        await _save_message(
            ...
            "assistant",
            text,
            {"source": "slash_command", "command": "project", "ok": True, "action": "switch"},
        )
        yield {"type": "project_switch", "project_id": target["id"], "title": target["title"], "action": "switched"}
        yield {
            "type": "slash_command",
            "command": "project",
            "action": "switch",
    ...
    if action in _PROJECT_DELETE_ACTIONS:
        async for event in _project_delete_events(project_id, command):
            yield event
        return

    text = _project_help_text(f"项目命令不合法：/{command.name} {action}")
    await _emit_text(project_id, command, text, ok=False)
    yield {"type": "slash_command", "command": "project", "action": action, "ok": False, "error": "invalid_project_action"}
    yield {"type": "text_delta", "content": text}
    yield {"type": "done", "status": "failed"}


async def _project_delete_events(
    project_id: str,
    command: SlashCommand,
) -> AsyncGenerator[dict[str, Any], None]:
    sub_action = (command.args[1].lower() if len(command.args) > 1 else "").strip()

    if sub_action in _PROJECT_DELETE_CONFIRM_ACTIONS:
        async for event in _project_delete_confirm_events(project_id, command):
            yield event
        return

    if sub_action in _PROJECT_DELETE_CANCEL_ACTIONS:
        _, state = await _read_state(project_id)
        if isinstance(state, dict):
            state.pop("_pending_project_delete_confirm", None)
            await _write_state(project_id, state)
        text = "已取消待确认的项目删除。"
        await _emit_text(project_id, command, text)
        yield {"type": "slash_command", "command": "project", "action": "delete_cancel", "ok": True}
        yield {"type": "text_delta", "content": text}
        yield {"type": "done", "status": "completed"}
        return

    query = " ".join(command.args[1:]).strip() or "current"
    projects = await _list_project_records()
    target, error_text = _resolve_project_record(projects, project_id, query)
    if not target:
        text = error_text or "未找到要删除的项目。用 /project 查看项目列表。"
        await _emit_text(project_id, command, text, ok=False)
        yield {"type": "slash_command", "command": "project", "action": "delete", "ok": False, "error": "project_not_found"}
        yield {"type": "text_delta", "content": text}
        yield {"type": "done", "status": "failed"}
        return

    pending = await _store_pending_project_delete(project_id, target)
    _emit_control_plane_event(
        project_id,
        command,
        branch="project_delete_confirmation_created",
        status="pending_confirmation",
        confirmation_kind="delete_project",
        risk="destructive",
        target_project_id=target["id"],
        title=target["title"],
        expires_at=pending.get("expires_at"),
    ...
    text = (
        f"删除项目需要确认：{target['title']}（{target['id']}）。"
        "该操作会删除项目记录，确认执行请发送 /project delete confirm；取消请发送 /project delete cancel。"
    )
    await _emit_text(project_id, command, text)
    yield {
        "type": "confirm_required",
...
async def _project_delete_confirm_events(
    project_id: str,
    command: SlashCommand,
) -> AsyncGenerator[dict[str, Any], None]:
    _, state = await _read_state(project_id)
    pending = state.get("_pending_project_delete_confirm") if isinstance(state, dict) else None
    if not pending:
        text = "没有待确认的项目删除。先发送 /project delete [current|项目id|标题]。"
        await _emit_text(project_id, command, text, ok=False)
        yield {"type": "slash_command", "command": "project", "action": "delete_confirm", "ok": False, "error": "no_pending_project_delete"}
        yield {"type": "text_delta", "content": text}
        yield {"type": "done", "status": "failed"}
        return
    if is_pending_confirmation_expired(pending):
        if isinstance(state, dict):
            state.pop("_pending_project_delete_confirm", None)
            await _write_state(project_id, state)
        text = "待确认的项目删除已过期。请重新发送 /project delete。"
        await _emit_text(project_id, command, text, ok=False)
        yield {"type": "slash_command", "command": "project", "action": "delete_confirm", "ok": False, "error": "confirmation_expired"}
        yield {"type": "text_delta", "content": text}
        yield {"type": "done", "status": "failed"}
        return

    target_id = str(pending.get("target_project_id") or "")
    target_title = str(pending.get("target_title") or target_id or "未命名项目")
    delete_result = await _delete_project_record(target_id)
    if not delete_result.get("ok"):
        text = f"删除失败：{delete_result.get('error') or 'Project not found'}"
        await _emit_text(project_id, command, text, ok=False)
        yield {"type": "slash_command", "command": "project", "action": "delete_confirm", "ok": False, "error": "project_not_found"}
        yield {"type": "text_delta", "content": text}
        yield {"type": "done", "status": "failed"}
        return

    deleted_current = target_id == project_id
    next_project = await _next_project_after_delete(target_id) if deleted_current else None
    text = f"已删除项目：{target_title}。"
    if deleted_current and next_project:
        text += f" 已切换到：{next_project['title']}。"
        await _save_message(
            next_project["id"],
            "user",
            command.raw,
        ...
        await _save_message(
            ...
            "assistant",
            text,
            {"source": "slash_command", "command": "project", "ok": True, "action": "delete_confirm"},
        )
    elif not deleted_current:
        _, fresh_state = await _read_state(project_id)
        if isinstance(fresh_state, dict):
            fresh_state.pop("_pending_project_delete_confirm", None)
            await _write_state(project_id, fresh_state)
        await _emit_text(project_id, command, text)
    ...
    _emit_control_plane_event(
        project_id,
        command,
        branch="project_deleted",
        status="completed",
        deleted_project_id=target_id,
        deleted_current=deleted_current,
        next_project_id=next_project.get("id") if next_project else None,
    )
    if next_project:
        yield {
            "type": "project_switch",
...
def _project_record(project: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "title": str(data.get("title") or "未命名项目"),
...
async def _list_project_records() -> list[dict[str, Any]]:
    async with session_scope() as session:
...
async def _create_project_record(title: str) -> dict[str, Any]:
    clean_title = (title or "").strip() or "未命名项目"
    async with session_scope() as session:
        svc = ProjectService(session)
        project = await svc.create_project(
            ...
            duration_per_episode=90,
            budget_level="low",
        )
    return _project_record(project)


async def _delete_project_record(project_id: str) -> dict[str, Any]:
    async with session_scope() as session:
        svc = ProjectService(session)
...
async def _next_project_after_delete(deleted_project_id: str) -> dict[str, Any]:
    projects = [project for project in await _list_project_records() if project["id"] != deleted_project_id]
    if projects:
        return projects[0]
    return await _create_project_record("未命名项目")
...
def _resolve_project_record(
    ...
    raw = (query or "").strip()
    if not raw or raw.lower() in {"current", "this", "当前", "本项目"}:
        for project in projects:
            if project["id"] == current_project_id:
                return project, None
        return None, "当前项目不存在。"

    index_text = raw[1:] if raw.startswith("#") else raw
    if index_text.isdigit():
        index = int(index_text)
        if 1 <= index <= len(projects):
            return projects[index - 1], None
        return None, f"项目序号超出范围：{index}。"

    id_matches = [project for project in projects if project["id"] == raw]
    if id_matches:
        return id_matches[0], None

    prefix_matches = [project for project in projects if project["id"].startswith(raw)]
    if len(prefix_matches) == 1:
        return prefix_matches[0], None
    if len(prefix_matches) > 1:
        return None, f"项目 id 前缀不唯一：{raw}。"

    folded = raw.casefold()
    exact_title = [project for project in projects if str(project["title"]).casefold() == folded]
    if len(exact_title) == 1:
        return exact_title[0], None
    if len(exact_title) > 1:
        return None, f"项目标题不唯一：{raw}。请用 id。"

    title_matches = [project for project in projects if folded in str(project["title"]).casefold()]
    if len(title_matches) == 1:
        return title_matches[0], None
    if len(title_matches) > 1:
        names = "，".join(project["title"] for project in title_matches[:5])
        return None, f"项目标题匹配到多个：{names}。请用序号或 id。"
    return None, f"未找到项目：{raw}。"


async def _store_pending_project_delete(project_id: str, target: dict[str, Any]) -> dict[str, Any]:
    _, state = await _read_state(project_id)
    pending = {
        "target_project_id": target["id"],
        "target_title": target["title"],
    ...
    state["_pending_project_delete_confirm"] = pending
    await _write_state(project_id, state)
    return pending


def _format_project_list(projects: list[dict[str, Any]], *, current_project_id: str) -> str:
    if not projects:
        return "当前没有项目。用 /project new [标题] 新建。"
    lines = ["项目列表："]
    for index, project in enumerate(projects[:20], start=1):
        marker = "当前" if project["id"] == current_project_id else "  "
        short_id = project["id"][:8]
        lines.append(f"{index}. [{marker}] {project['title']} `{short_id}`")
    if len(projects) > 20:
        lines.append(f"... 还有 {len(projects) - 20} 个项目。")
    lines.append("命令：/project new [标题]，/project switch <序号|id|标题>，/project delete [current|序号|id|标题]。")
    return "\n".join(lines)


def _project_help_text(prefix: str | None = None) -> str:
    lines = []
    if prefix:
        lines.append(prefix)
    lines.extend([
        "项目命令：",
        "- /project：查看项目列表",
        "- /project new [标题]：新建项目并切换",
        "- /project switch <序号|id|标题>：切换项目",
        "- /project delete [current|序号|id|标题]：请求删除确认",
        "- /project delete confirm：确认删除",
        "- /project delete cancel：取消删除",
    ])
    return "\n".join(lines)
