...
- `visual_plan_mode`：画面制作模式，可选 `storyboard`（宫格分镜）或 `story_template`（故事模板），默认 `storyboard`。
...
- `storyboard`：宫格分镜模式下的当前段宫格分镜图，显示在画布。
- `story_template`：故事模板模式下的当前段专业故事板设计图，显示在与宫格分镜对应的视觉产物位置。
- `final_video` / `final_video_story_template`：两条互斥分支的当前段最终视频节点。宫格分镜分支读取通过审核的 `storyboard`；故事模板分支真实查看通过审核的 `story_template` 并逐格转译，不能另编模板图中不存在的内容。

`final_video.execution` 决定流程运行方式：`auto` 会继续生成，`manual` 只准备视频提示词和画布节点，等待用户在视频节点上点击生成。流程编辑器中的“手动运行”直接保存为这一字段。

规划类步骤如 `episode_plan`、`production_plan`、`scene_plan`、`frame_plan`、`storyboard_review` 和 `story_template_review` 保存在运行态，供下游节点读取。`script` 和每个 `segment_script` 的可读正文会同步到画布文本节点；两条最终视频分支的公开 `prompt` 分别编译为私有提示词阶段；集合类结构由后端根据步骤 schema 自动处理。
...
- 两条条件分支都使用固定整数 `foreach.count` 限制最大尝试次数，并用 `foreach.until` 读取本轮终点审核步骤的声明输出字段；宫格分镜读取 `steps.storyboard_review.output.score >= 80`，故事模板读取 `steps.story_template_review.output.score >= 80`。
...
- `story_template` 使用相同反馈合同，但审核重点换成镜头顺序与时长、故事板主体区、空间机位与轴线、人物场景锚点、参考一致性和标注可读性。最终视频严格按实际模板图逐格转译。
- 依赖保持正向：候选产物到审核步骤，不添加审核到候选产物的公共反向边。运行器负责逐轮串行、反馈注入和选择最后一轮通过产物。
- 达标后匹配模式的最终视频及其私有提示词阶段依赖整个循环并使用通过轮次。最大次数仍未达标、门控字段缺失或类型无效时停止下游，不把无效结果当成普通低分继续运行。
