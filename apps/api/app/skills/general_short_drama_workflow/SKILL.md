...
图片步骤的 `fields.aspect_ratio` 默认固定为 `16:9`，流程编辑器直接选择明确比例；视频步骤的 `fields.aspect_ratio` 使用 `{{ inputs.aspect_ratio }}`，投影和运行时读取本次视频画幅。剧本和制作规划必须让首段从 0 秒开始、相邻段首尾相接、末段精确结束于 `duration_seconds`，各段时长之和与总时长完全一致。
