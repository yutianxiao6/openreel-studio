...
模板只保存可复用结构：输入字段、步骤、依赖、展开规则、引用关系、prompt skill 引用、每一步的 `prompt_template` 和验收标准。本次剧情、剧本文字、人物描述、图片 prompt、视频 prompt 成品在运行实例中生成，不写死进模板。
...
节点运行阶段以模板里的 `prompt_template` 为准。需要局部改提示词时，修改对应步骤的 `prompt_template`，再重跑该步骤及受影响下游。
